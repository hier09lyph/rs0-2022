...
function greetingPlaceholder(lang) {
    greetingContainer.querySelector('input').placeholder = lang === 'ru' ? '[??????? ???]' : '[Enter name]';
}
...
function defaultCity() {
    if (city.value === '') {
        city.value = lang === 'ru' ? '?????' : 'Minsk';
    } else if (city.value === '?????' || city.value === 'Minsk') {
        city.value = lang === 'ru' ? '?????' : 'Minsk';
    }
}
...
async function getWeather() {
    ...
    try {
        ...
        temperature.textContent = `${Math.trunc(data.main.temp)}�C`; //??????????? ? �C
        weatherDescription.textContent = data.weather[0].description; //???????? ??????

        if (lang === 'ru') {
            wind.textContent = `????. ?????: ${Math.trunc(data.wind.speed)}?/?`; //???????? ????? ? ?/?
            humidity.textContent = `????. ???????: ${Math.trunc(data.main.humidity)}%`; // ????????????? ????????? ??????? ? %
        } else {
            wind.textContent = ` Wind speed: ${Math.trunc(data.wind.speed)}?/?`; //???????? ????? ? ?/?
...
let popup = document.querySelector('.popup'); // ???? ???? loginPopUp
let openPopupButton = document.querySelectorAll('.setting__link'); // ?????? ??? ?????? ????
...
loginPopUpSignIn.forEach((button) => {
    button.addEventListener('click', (e) => {
        ...
        alert(`?? ?????: \n ?????: ` + loginPopUpEmail.value + `\n ??????: ` + loginPopUpPassword.value);
    })
});
...
signUpPopUpSignIn.forEach((button) => {
    button.addEventListener('click', (e) => {
        ...
        alert(`?? ?????: \n ?????: ` + createPopUpEmail.value + `\n ??????: ` + createPopUpPassword.value);
    })
});
